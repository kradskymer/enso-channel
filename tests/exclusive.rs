...
impl Channel for Spsc {
    type Sender = enso_channel::exclusive::spsc::Sender<u32>;
    type Receiver = enso_channel::exclusive::spsc::Receiver<u32>;

    fn channel(capacity: usize) -> (Self::Sender, Self::Receiver) {
        enso_channel::exclusive::spsc::channel::<u32>(capacity)
    }
...
impl h::ExclusiveChannel for Spsc {
...
impl harness::contracts::InduceUncommittedClaim for Spsc {
...
impl Channel for Mpsc {
    type Sender = enso_channel::exclusive::mpsc::Sender<u32>;
    type Receiver = enso_channel::exclusive::mpsc::Receiver<u32>;

    fn channel(capacity: usize) -> (Self::Sender, Self::Receiver) {
        enso_channel::exclusive::mpsc::channel::<u32>(capacity)
    }
...
impl h::ExclusiveChannel for Mpsc {
    fn try_send_batch_write_exact(
        sender: &mut Self::Sender,
        items: &[u32],
    ) -> Result<(), enso_channel::errors::TrySendError> {
...
impl h::CloneSender for Mpsc {
    fn clone_sender(sender: &Self::Sender) -> Self::Sender {
        sender.clone()
    }
}

impl harness::contracts::InduceUncommittedClaim for Mpsc {
    fn induce_uncommitted_claim(sender: &mut Self::Sender, n: usize) {
        fn panic_factory() -> u32 {
            panic!("intentional panic to skip publish")
        }
...
#[test]
fn spsc_send_batch_write_exact() {
    h::send_batch_write_exact::<Spsc>();
}

#[test]
fn mpsc_send_batch_write_exact() {
    h::send_batch_write_exact::<Mpsc>();
}

#[test]
fn mpsc_sender_is_cloneable() {
    h::sender_is_cloneable::<Mpsc>();
}
...
impl harness::concurrency::CloneSender for Mpsc {
    fn clone_sender(sender: &Self::Sender) -> Self::Sender {
        sender.clone()
    }
}
...
fn spsc_concurrent_producer_consumer() {
    harness::concurrency::contract_concurrent_spsc::<Spsc>();
}

#[test]
fn mpsc_concurrent_producer_consumer() {
    harness::concurrency::contract_concurrent_spsc::<Mpsc>();
}

#[test]
fn mpsc_concurrent_multi_producer() {
    harness::concurrency::contract_concurrent_mpsc::<Mpsc>(4);
}
